...
struct _Complex_Traits_Base
{
    ...
    static bool signbit(float_type val) { // has all floating point overloads
        return std::signbit(val);
    }

    static float_type abs_v(float_type val) { // has all floating point overloads
        return ::abs(val);
    }
...
struct _Complex_Traits_Float : _Complex_Traits_Base<float>
{
    ...
    static float_type hypot(float_type left, float_type right) { // return sqrt(left ^ 2 + right ^ 2)
        return ::hypotf(left, right);
    }

    static float_type exp(float_type val) { // return e ^ val
        return ::expf(val);
    }

    static float_type ldexp(float_type val, int exponent) { // return val * 2 ^ exponent
        return ::ldexpf(val, exponent);
    }

    static float_type log(float_type val) { // return log_e (val)
        return ::logf(val);
    }

    static float_type log1p(float_type val) { // return log_e (1 + val)
        return ::log1pf(val);
    }
...
public:
    ...
    constexpr complex& operator=(const complex<_Type>& other) {
        _rep[Real] = static_cast<value_type>(other.real());
        _rep[Imag] = static_cast<value_type>(other.imag());
        return *this;
    }
...
private:
    ...
    constexpr void _mul(const complex<_Type>& other) {
        ...
        value_type temp      = _rep[Real] * otherReal - _rep[Imag] * otherImag;
    ...
    constexpr void _div(const complex<_Type>& other) {
        ...
        else if (traits_type::abs_v(otherImag) < traits_type::abs_v(otherReal))
        {
            ...
            else
            {
                ...
                value_type temp  = (_rep[Real] + _rep[Imag] * wr) / wd;
        ...
        else // 0 < |other._rep[Real]| <= |other._rep[Imag]|
        {
            ...
            {
                // compute representable result
                value_type temp  = (_rep[Real] * wr + _rep[Imag]) / wd;
